...
def extractMainLobe(window, M):
    ...
    ### Your code here
    mx=20*np.log10(abs(fftshift(fft(w,M*8))))

    maxima=np.argmax(mx)
    local_minima_left = maxima
    while mx[local_minima_left] >= mx[local_minima_left-1] and local_minima_left > 0:
        local_minima_left -= 1
    local_minima_right = maxima
    while mx[local_minima_right] >= mx[local_minima_right+1] and local_minima_right < len(mx):
        local_minima_right += 1
    return mx[local_minima_left:local_minima_right+1]
