...
def IDFT(X):
    ...
    for k in xrange(N):
        sum_k = 0
        # I do this with this loop
        # sum_k = np.sum( np.prod( [ x, np.exp( -1j * 2.0 * np.pi * np.arange(0, N, 1) * k / N ) ], axis = 0 ) )
        for t in xrange(N):
            sum_k += (X[t] * np.exp( 1j * 2.0 * np.pi * k * t / N ))
            
        x_idft = np.append( x_idft, sum_k/N )
        print sum_k/N
    """

    #return x_dft
    return np.fromiter( ((1.0/N)*(np.sum( np.prod( [ X, np.exp( 1j * 2.0 * np.pi * k * np.arange( 0, N, 1 ) / N ) ], axis = 0 ) )) for k in np.arange(0, N, 1)), np.complex )
