...
def genMagSpec(x):
    ...
    """
    ## Your code here
    # see part 3 for description of this line
    N = len (x)
    return np.fromiter( ((np.abs(np.sum( np.prod( [ x, np.exp( -1j * 2.0 * np.pi * k * np.arange( 0, N, 1 ) / N ) ], axis = 0 ) ))) for k in np.arange(0, N, 1)), np.float )
